...
class MusicApi
{
    ...
    // 音频数据接口地址
    public function mc_song_urls($value, $type = 'query', $site = 'netease', $page = 1)
    {
        if (!$value) {
            return;
        }
        ...
        $radio_song_urls = [
            ...
            'qingting' => [
                'method' => 'GET',
                'url' => 'http://i.qingting.fm/wapi/channels/' . $this->split_songid($songid, 0) . '/programs/' . $this->split_songid($songid, 1),
                'referer' => 'http://www.qingting.fm',
                'proxy' => false,
                'body' => false
            ],
    ...
    // 获取音频信息 - 关键词搜索
    public function mc_get_song_by_name($query, $site = 'netease', $page = 1)
    {
        if (!$query) {
            return;
        }
        $radio_search_url = $this->mc_song_urls($query, 'query', $site, $page);
        ...
        return $this->mc_get_song_by_id($radio_songid, $site, true);
    }

    // 获取音频信息 - 歌曲ID
    public function mc_get_song_by_id($songid, $site = 'netease', $multi = false)
    {
                ...
                foreach ($radio_result as $val) {
                    ...
                    if (!empty($radio_data)) {
                        foreach ($radio_data as $value) {
                            ...
                            if ($radio_lrc_urls) {
                                $radio_lrc = $this->mc_curl($radio_lrc_urls);
                            }
                            $radio_songs[] = [
                                'type' => '1ting',
                ...
                foreach ($radio_result as $val) {
                    ...
                    if (!empty($radio_data)) {
                        foreach ($radio_data as $value) {
                            ...
                            $radio_lrc_urls = $this->mc_song_urls($radio_song_id, 'lrc', $site);
                            if ($radio_lrc_urls) {
                                $radio_lrc = json_decode($this->mc_curl($radio_lrc_urls), true);
                            }
                            $radio_songs[] = [
                                'type' => 'baidu',
            ...
            case 'kuwo':
                foreach ($radio_result as $val) {
                    ...
                    if (!empty($radio_json[1]) && !empty($radio_json[2])) {
                        ...
                        $radio_lrc_urls =$this-> mc_song_urls($radio_song_id, 'lrc', $site);
                        if ($radio_lrc_urls) {
                            $radio_lrc_info = json_decode($this->mc_curl($radio_lrc_urls), true);
                        }
                        ...
                        $radio_songs[] = [
                            ...
                            'lrc' => $this->generate_kuwo_lrc($radio_lrclist),
                ...
                break;
            case 'qq':
                $radio_vkey = json_decode($this->mc_curl([
                    'method' => 'GET',
                ...
                foreach ($radio_result as $val) {
                    ...
                    if (!empty($radio_data)) {
                        foreach ($radio_data as $value) {
                            ...
                            $radio_song_id = $value['songId'];
                            if ($value['lyric']) {
                                $radio_lrc_urls = $this->mc_song_urls($value['lyric'], 'lrc', $site);
                                if ($radio_lrc_urls) {
                                    $radio_lrc = $this->mc_curl($radio_lrc_urls);
                                }
                            }
                            $radio_songs[] = [
                                ...
                                'lrc' => $radio_lrc,
                                'url' => $this->decode_xiami_location($value['location']),
                                'pic' => $value['album_pic']
                            ];
                        }
                    } else {
            ...
            case 'qingting':
                foreach ($radio_result as $val) {
                    ...
                    if (!empty($radio_data)) {
                        ...
                        $radio_info = json_decode($this->mc_curl($radio_channels), true);
            ...
            case 'kg':
                foreach ($radio_result as $key => $val) {
                    ...
                    if (!empty($radio_data)) {
                        ...
                        $radio_lrc_urls = $this->mc_song_urls($radio_data['ksong_mid'], 'lrc', $site);
                        if ($radio_lrc_urls) {
                            $radio_lrc_info = json_decode($this->mc_curl($radio_lrc_urls), true);
                        }
                        $radio_songs[] = [
    ...
    // 获取音频信息 - url
    public function mc_get_song_by_url($url)
    {
        ...
        if (!empty($match_netease)) {
            ...
            return $this->mc_get_song_by_name($match_kg_uid[2], 'kg');
        } else {
            return;
        }
        return $this->mc_get_song_by_id($songid, $songtype);
    }

    // 解密虾米 location
    public function decode_xiami_location($location)
    {
    ...
    // 分割 songid 并获取
    public function split_songid($songid, $index = 0, $delimiter = '|')
    {
    ...
    // Response
    public function response($data, $code = 200, $error = '')
    {
